...
tags: ['javascript']
date: '2025-01-28'
description: '프로토타입의 동작 원리와 함께 [object Object]에 대해 알아보고, [object Object]를 직접 바꾸고 활용하는 내용을 다뤘다.'
...
위 코드를 보면, **_obj_** 를 출력했을 때는 객체의 내용을 그대로 보여주지만, 문자열 `''`를 더한 경우 **_\[object Object]_** 문자열로 출력된다. 이는 객체가 암묵적으로 문자열 타입 변환이 된 결과다.

그렇다면 객체가 문자열로 변환될 때 내부적으로 어떤 일이 일어날까? 자바스크립트에서 객체가 문자열로 변환될 때, 기본적으로 객체의 **_toString_** 메서드가 호출된다. **_toString_** 메서드는 객체를 `[object 타입]` 형식으로 반환하기 때문에 **_\[object Object]_** 라는 결과가 나온 것이다.

만약 객체를 그대로 문자열로 출력하고 싶다면, `JSON.stringify` 를 사용할 수 있다. 이 메서드는 객체를 JSON 문자열로 객체 그대로 반환한다.
...
사실 **_toString_**은 나에게 꽤 친숙한 메서드다. 아마 여러분들도 Java와 같은 객체지향 언어를 공부한 경험이 있다면 익숙할 것이다.
...
Java에서 **_toString_**은 하위 클래스가 직접 구현하지 않아도 사용할 수 있는 메서드다. 이는 객체지향 프로그래밍 언어의 특징으로, 모든 클래스가 상위 클래스인 Object 로부터 **_toString_** 메서드를 상속받기 때문이다. 즉, 직접 정의하지 않아도 상위 클래스에서 상속받은 메서드를 사용할 수 있는 구조다.

자바스크립트에서도 동일하다. 자바스크립트의 객체는 프로토타입 객체를 통해 상위 객체의 속성과 메서드를 상속받는다. 따라서 객체를 생성할 때, 그 객체는 기본적으로 `Object.prototype` 에 정의된 **_toString_** 메서드를 상속받는다. 이를 통해 **_toString_** 을 직접 정의하지 않아도 사용할 수 있는 것이다.
...
쉽게 말해, 프로토타입 객체는 상위 객체가 하위 객체에게 공유할 프로퍼티를 저장할 객체라고 보면 된다. 이때 상위 객체의 프로토타입 객체와 하위 객체를 연결하고 있는 것이 바로 **_prototype chain_** 이다.
...
프로토타입 객체 내부에 **_toString_** 이 있는 것을 볼 수 있다.
...
- **_\[object Object]_** 는 객체가 문자열로 평가되어 실행된 결과다
- 객체가 문자열로 평가될 때, **_toString_** 이 실행된다.
- **_toString_** 은 프로토타입 객체를 통해 최상위 부모인 **_Object_** 로부터 받아 실행하고 있다.
...
주의할 점은, 직접 사용 시 this 값을 명시적으로 바인딩해줘야 한다는 것이다. 함수가 정의된 위치를 기준으로 this는 전역객체가 되기 때문이다. 따라서 `call` 를 이용하자. this 를 인자값으로 참조할 수 있도록 말이다.
