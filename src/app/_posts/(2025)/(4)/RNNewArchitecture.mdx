...
title: 'React Native 렌더링 병목, v0.68~ 도입된 New Architecture로 어떻게 해결했을까?'
description: 'React Native 의 기존 Bridge 기반 아키텍처가 어떻게 렌더링 병목을 초래했는지, 그리고 v0.70부터 도입된 New Architecture 가 이를 구조적으로 어떻게 해결했는지 ***JSI*** 와 Fabric Renderer*** 관점에서 살펴봅니다.'
tags: [ReactNative]
date: '2025-04-10'
---
...
그래서 **_React Native_** 팀은 기존의 **_bridge_** 기반 구조를 넘어, 완전히 새로운 아키텍처를 도입하면서 이 문제를 해결했습니다.
...
![](https://github-production-user-asset-6210df.s3.amazonaws.com/127009780/432155061-4191bd5b-9e53-4d3c-a4a8-b74d8bdb94e8.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAVCODYLSA53PQK4ZA/20250410/us-east-1/s3/aws4_request&X-Amz-Date=20250410T064542Z&X-Amz-Expires=300&X-Amz-Signature=9936d607e617bc1c12815e21825513e41e44eb0b64badc69c95c27eacc82d94c&X-Amz-SignedHeaders=host)
...
병목은 이 과정에서 발생합니다. 위에서 세 Thread 는 모두 비동기로 동작한다고 했어요. 만약 Native Thread 에서 event 가 한꺼번에 너무 많이 발생한다면 어떨까요? 그 와중에 JS Thread 가 바쁘다면? Thread 들은 서로 비동기적으로 동작하기 때문에 드래그 애니메이션처럼 많은 이벤트를 발생시키는 경우 Native Thread 는 한꺼번에 수많은 evnet 들을 JS Thread 로 보낼겁니다. JS Thread 는 처리하는게 정신없을 거고 그렇게 프레임 드랍, 성능 문제가 발생하게 돼요.
...
![](https://github-production-user-asset-6210df.s3.amazonaws.com/127009780/432159592-609fea62-a48f-4733-ae7d-c84aa7d2d0b6.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAVCODYLSA53PQK4ZA/20250410/us-east-1/s3/aws4_request&X-Amz-Date=20250410T065705Z&X-Amz-Expires=300&X-Amz-Signature=221c25dc7292ab6972ec29dce7d0a2f4e4b474ad01bad3d828826b517dd664ee&X-Amz-SignedHeaders=host)
세 가지 핵심 기술이 도입되었어요.
...
참고로 그림에서 나온 Yoga 는 Yoga Tree를 의미합니다. Shadow Tree 는 레이아웃 정보를 포함하는데, 이때 레이아웃 정보를 갖고 있는 공간이 Shadow Node 내부의 Yoga Node 에요. 그리고 이러한 Yoga Node 들의 집합을 Yoga Tree 라고 합니다. Yoga Tree 는 레이아웃을 계산하는 주체에요.
