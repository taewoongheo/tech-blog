...
따라서 다시 `onRefresh` 를 `SensorData` 컴포넌트 내부로 이동시켰습니다. 결론적으로 다시 돌아왔기 때문에 바뀐 것은 없지만, 맥락을 파악하는 과정 위주로 봐주시면 감사하겠습니다.
...
25.03.19추가) 좀 더 개선한다고 하면, onRefresh의 try~catch 부분이 드러난 구현세부를 refreshing의 변경책임을 보여주며 추상화할 수 있을 것 같아요. 이 부분은 계속 고민하며 수정해볼 예정입니다.
